...
export async function newPost(noAi: boolean = false): Promise<void> {
  ...
  try {
    ...
    if (!config.github.owner || !config.github.repo) {
      outro(
        "❌ GitHub configuration missing. Check GITHUB_URL or GITHUB_OWNER + GITHUB_REPO in .env",
      );
      return;
    }
    ...
    if (noAi) {
      console.log(
        "⚙️  Testing mode: Skipping AI formatting, using raw content...",
      );
      content = rawText;
    } else {
    ...
    if (existingFile) {
      ...
      note(
        "Choose a different title or use blog-cli list to see existing posts.",
        "Tip",
      );
      return;
    }
    ...
    outro(
      `   GitHub: https://github.com/${config.github.owner}/${config.github.repo}/blob/${config.github.branch}/${filePath}`,
    );
  } catch (error) {
